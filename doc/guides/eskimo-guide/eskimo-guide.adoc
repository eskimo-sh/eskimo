...
* an Administration Application aimed at drastically simplifying the deployment, administration and operation of your
  Big Data Cluster
* a state of the art Big Data 2.0 platform based on Docker, Mesos and Systemd, and packaging *Gluster, Spark, Kafka,
  and ElasticSearch*
...
Three components are avaoilable in the storage layer
...
* Spark : the large scale very versalitle computation engine
* As a sidenote, ElasticSearch can also be considered part of the processing tier since it provided many processing
...
* CentOS 7.x and 8.x
* Fedory 29 and greater

===== Firewall open ports on eskimo nodes

In case a firewall (firewalld or simple iptables conofiguration) is installed on eskimo cluster nodes, then
...
advised to pen them externally as well.

===== Eskimo system user

Eskimo qill require a system user properly defined and with SSH access to reach and operate the cluster nodes.
...
*In addition, that user needs to have sudo access without the need to type a password!*


==== Internet access on cluster nodes

Eskimo performes some initial setup operations on every node of the cluster it needs to operate. Some of these
...
managed by eskimo needs to have granted access using SSK Public Key authentication to this user.
...
One of the most essential screen of the Eskimo Web Console, the one which is reach just after login, the the status
screen.

This is an example of the status screen showing a three nodes cluster and the services installed on a cluster.
...
* [purple]#violet# : the service is pending removal from the node.
...
service context menu which he can use to stop / start / restart or even fully reinstall a service.
...
actually have the very same configuration.
...
for the time being, administrator need to configure eskimo using IP addresses and only IP addresses.
...
These "_Master services_" - considered unique - can only be configured in single node configurations and only once:

image::pngs/unique-services.png[600, 600, align="center"]

==== Slave services

Sme other services are considered *slave services* and can be deployed at will, on one single or all nodes of the
cluste (understand services that can be deployed multiple toimes, e.g. elasticsearch, kafka, mesos-agent, etc.) and
...
The last and last but one menu entries in the "_Eskimo Services_" part are special consoles implennted within eskimo to
...
* `MOUNT_POINT` is the foldre where to mount that volume on the local filesystem.

The beauty of this script is that it takes care of everything:

* Registering the local node with the gluster cluster of not already done
...
* `/var/lib/spark/data` where spark stored its own data but the user can store his own data to be used accross spark
executors as well
* `/var/lib/spark/eventlog` where the spark executors and the spark drive store their log and used by the spark history
server to monitor spark jobs.
