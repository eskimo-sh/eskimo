...
* A service is mostly a docker image and the scripts aimed at installing and setting it up.
* Everything - from building the docker image to installing it on cluster nodes - is done using simple shell scripts.
With Eskimo a system administrator desiring to leverage on eskimo to implement his own service doesn't need to need any
new and fancy technology, just plain old shell scripting, docker and systemd.
...
include::../../../packages_dev/README.adoc[leveloffset=0,lines=60..160]
