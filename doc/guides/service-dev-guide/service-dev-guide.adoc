...
This documentation is related to the last of the key features presented above : The
Services Framework. +
It presents everything a developer needs to understand and know to develop his own
services and let eskimo distribute and operate them, or extend current services.
...
nodes
* The <<services_installation_framework>> which is used to install these images as services on the eskimo cluster nodes.
...
** a docker image packaging the software component and
** the set of scripts and systemd configuration required to install it, set it up and operate it
* Everything - from building the docker image to installing it on cluster nodes - is done using simple shell scripts.
With Eskimo a system administrator desiring to leverage on eskimo to implement his own services doesn't need to learn
any new and fancy technology, just plain old shell scripting, docker and systemd. Period.
...
include::../../../packages_dev/README.adoc[leveloffset=0,lines=60..180]



// included service developer README

include::../../../services_setup/README.adoc[leveloffset=0,lines=60..310]
