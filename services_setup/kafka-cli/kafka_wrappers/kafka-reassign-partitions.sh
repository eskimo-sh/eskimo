...
for argument in "$@"; do
    ...
    if [[ $argument == "--topics-to-move-json-file" || $argument == "--reassignment-json-file" || $argument == "--command-config" ]]; then
...
/usr/bin/docker run \
        ...
        kubernetes.registry:5000/kafka \
            /usr/local/bin/kube_do /usr/local/sbin/kafka-reassign-partitions.sh "$@"
