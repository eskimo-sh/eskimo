...
back to the eskimo UI and reinstall a service. +
The tool takes the form of a shell script and is named `eskimo-edit-image.sh`.
...
It takes as argument the name of the service (wich corresponds to the docker image name once a service is installed) as
argument as well as an optional shell script aimed at performing the cusomization. +
If the customization script is ommited, then a shell is presented to the user for him to perform the customization
interactively.

This would be the way to make `vim` available in the `cerebro` container for instance :
...
As a result of the above sequence, a new tag version "2" will be available in the system and the user is
left with restarting cerebro manually on Kubernetes, which can be done with the help of the `eskimo-kubectl`
command:
...
// marker for exclusion : line 1000
