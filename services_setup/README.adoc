...
SystemD services rely on the host to mount gluster shares and then mount the share to the gluster containre from the
host mount. +
...
* The service SystemD unit file should define a dependency on the SystemD mount by usingf the following statements +
...
The approach is very similar for marathon services, but not relying on SystemD which is not available from within
containers. +
So marathon services actually mount the gluster share directly from within the docker container. +
The way to do this is as follows:
