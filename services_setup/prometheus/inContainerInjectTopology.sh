...
bash -c "echo -e \"    # targets come from /etc/eskimo_topology.sh - ALL_NODES_LIST_kube_slave\" >> /usr/local/lib/prometheus/prometheus.yml"
bash -c "echo -e \"    - targets:\" >> /usr/local/lib/prometheus/prometheus.yml"
for i in $(echo "$ALL_NODES_LIST_kube_slave"  | tr "," "\n"); do
