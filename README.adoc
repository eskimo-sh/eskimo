...
Every other dependency will be downloaded during the maven build process (except `trilead-ssh2`, see <<trilead-ssh2>> below).
...
Whe nsuch think happen the building of the corresponding local image will fail in Eskimo. +
...
.integration-test.sh
----
badtrash@badbook:/work/eskimo/test_lab/integration_test$ ./integration-test.sh
Usage:
...
----
badtrash@badbook:/work/eskimo/test_lab/integration_test$ ./integration-test.sh -ad
----
