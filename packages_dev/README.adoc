...
In order for one to be able to either build the eskimo software compontnents packages locally (or have eskimo building
...
When runnign eskimo, software packages - either service docker images or mesos binary packages - can be either built
...
It's content should be aligned with the followin properties from the configuration file `eskimo.properties`:
